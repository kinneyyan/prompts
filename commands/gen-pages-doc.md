---
description: 为 src/pages 目录下的页面文件自动添加文件头注释并生成 README.md 文档。解析页面内容提取组件、接口和服务信息，支持多种菜单配置路径（ice.js 2/3），自动识别用户路径，最终生成处理报告
allowed-tools: Bash(npx @bud-fe/docs-gen-cli:*)
---

通过 `@bud-fe/docs-gen-cli` 提供的脚本获取需要遍历 `src/pages` 下的页面路径，然后依次解析每个页面文件：1. 检查是否有给定的格式的文件头注释，如果没有则分析文件添加；2. 检查和页面文件平级的目录下是否有 `README.md`，如果没有则根据给定的格式生成。输入为 `src/pages` 下的页面路径列表，输出为更新的文件头注释和生成的 `README.md` 文件。

## 1. 初始化和环境验证

1. 检查 `src/pages` 目录是否存在并包含页面文件。

2. 询问用户项目的菜单配置&页面路由文件的路径：
   - 选项 1：`src/menuConfig.tsx`【for ice.js 3】
   - 选项 2：`src/layouts/basic-layout/menuConfig.ts` 和 `src/routes.ts`【for ice.js 2】
   - 选项 3: 其他自定义路径
   - 选项 4: 忽略

3. 根据用户的选择，读取指定的菜单配置&页面路由文件，提取并理解关于菜单&页面路由之间的关系图，为后续生成页面文件的 `README.md` 中的“用户路径”内容作准备（若用户选择“忽略”则留空或标记为待补充）。

## 2. 获取需要添加注释的页面路径列表并依次添加注释

1. 执行 `@bud-fe/docs-gen-cli` 这个包提供的 node 脚本，此脚本会扫描指定目录下的所有 `.tsx` 文件，输出所有没有约定文件头注释的页面文件路径列表：

   ```bash
   npx @bud-fe/docs-gen-cli ls-pages src/pages --no-comment
   ```

2. 解析命令输出，获取需要遍历的页面文件路径列表。

3. 遍历每个页面文件路径，对于每个页面文件：
   1. 读取页面文件内容
   2. 分析文件内容确定功能模块和页面名称，在文件开头添加适当的注释

## 3. 获取需要生成文档的页面路径列表并依次生成文档

1. 执行 `@bud-fe/docs-gen-cli` 这个包提供的 node 脚本，此脚本会扫描指定目录下的所有 `.tsx` 文件，输出所有没有对应 `README.md` 的的页面文件路径列表：

   ```bash
   npx @bud-fe/docs-gen-cli ls-pages src/pages --no-readme
   ```

2. 解析命令输出，获取需要遍历的页面文件路径列表。

3. 遍历每个页面文件路径，对于每个页面文件的同级目录：
   1. 检查是否存在 `README.md` 文件
   2. 如果不存在 `README.md`，分析页面文件内容提取以下信息：
      - 使用的组件（通过 import 语句分析，主要关注 UI 组件）
      - 调用的接口服务地址（通过分析引入的 `@/services/*` 的模块内的内容，接口地址一般为 `@/services/*` 内 request 实例调用的 `get`、`post` 方法的第一个参数）
      - 页面功能概述
   3. 创建 `README.md` 文件，遵循以下格式：

      ```md
      ## {{ kebabCase name }}

      ### 概述

      页面功能概述

      ### 用户路径

      首页 ➡️ xxx ➡️ xxx

      ### 使用组件

      - `src/components/auth`: 控制用户角色按钮权限
      - `src/components/page-header`: 页面头部标题
      - `@bud-fe/react-pc-ui`
        - `BfFormTable`: 筛选-列表

      ### 接口服务

      - `/api/users`: 获取用户列表

      ### PRD 文档

      - 待补充
      ```

## 5. 验证和报告生成

1. 通过执行 `npx @bud-fe/docs-gen-cli ls-pages src/pages --no-comment` 和 `npx @bud-fe/docs-gen-cli ls-pages src/pages --no-readme` 两个命令，检查是否还有未生成注释或者文档的页面文件

2. 向用户展示处理报告，包括：
   - 成功添加注释的文件数量
   - 成功生成 README.md 的文件数量
   - 跳过的文件数量和原因
   - 失败的文件数量和错误信息
